...
@app.route('/api/aadhar_extraction', methods=['GET','POST'])
def aadhar_extraction():
    mode_executed = "OCR-MASKING"
    flag_mask = 0
    r = request.get_json(force=True)  #force=True #content type application/json
    temp_name = "temp_unmasked.png" 
    image = r['doc_b64_image'] # raw data with base64 encoding
    decoded_data = base64.b64decode(image)
    np_data = np.fromstring(decoded_data,np.uint8)
    img = cv2.imdecode(np_data,cv2.IMREAD_UNCHANGED)
    cv2.imwrite(temp_name,img)
    aadhaar_list = ac.extract_aadhar(temp_name)
    content_type = 'application/json'
    headers = {'content-type': content_type}
    write = "temp_unmasked.png"
    img_bytes = to_image_string(write)
    ## delete_file(write)
     #convert byte to string
    encoded_string = img_bytes.decode("utf-8")
    # return Response(response=jsonpickle.encode({'doc_b64_image':encoded_string, 'is_masked': True,'mode_executed' : mode_executed}), status=200, mimetype="application/json", headers=headers)
