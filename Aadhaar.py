...
class Aadhaar_Card():
    ...
    def extract(self, path):  #("path of input image")
        ...
        if self.config['orient']:
            ...
            cv2.imshow('self.cv_img',cv2.resize(self.cv_img,(500,500)))
            cv2.waitKey(0)

            # cv2.imshow('img',cv2.resize(self.cv_img,(500,500)))
    ...
    def mask_image(self, path, write, aadhaar_list):
        ...
        # self.image_path = path
        # self.read_image_cv()
        # self.mask = self.rotate(self.cv_img)
    ...
    def rotate(self,img):
        ...
        # cv2.imshow('img_rotated',cv2.resize(img_rotated,(500,500)))
        # cv2.waitKey(0)
    ...
    def extract_aadhar(self, path):  #("path of input image")
        ...
        foo.save("image_scaled.jpg",optimize=True,quality=60)

        rotate_jpeg('image_scaled.jpg')

        image = cv2.imread('image_scaled.jpg')

        output = reader.readtext('image_scaled.jpg',detail = 0)
